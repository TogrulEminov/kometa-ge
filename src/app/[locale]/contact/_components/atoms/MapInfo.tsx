...
export default function MapInfo({
  ...
  const latitude = contactInfo?.latitude
    ? parseFloat(contactInfo?.latitude)
    : 0;
  const longitude = contactInfo?.longitude
    ? parseFloat(contactInfo?.longitude)
    : 0;

  return (
    <div className="flex items-start gap-3">
      <div className="w-10 h-10 bg-surface-elevated rounded-xl flex items-center justify-center text-primary shrink-0">
