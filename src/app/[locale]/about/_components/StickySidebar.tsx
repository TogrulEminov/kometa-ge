"use client";

import { availableIcons, renderSocialIcon } from "@/utils/renderSocialIcon";
import { motion } from "framer-motion";

interface NavItem {
  label: string;
  href: string;
}

const navItems: NavItem[] = [
  { label: "Haqqımızda", href: "#section-about" },
  { label: "Biz kimik?", href: "#section-who" },
  { label: "Fəaliyyət coğrafiyamız", href: "#section-geo" },
  { label: "Beynəlxalq filiallar", href: "#section-branches" },
  { label: "Tez-tez verilən suallar", href: "#section-faq" },
  { label: "Xidmətlərimiz", href: "#section-services" },
];

const contactInfo = [
  {
    icon: (
      <svg
        className="w-5 h-5"
        fill="none"
        stroke="currentColor"
        viewBox="0 0 24 24"
      >
        <path
          strokeLinecap="round"
          strokeLinejoin="round"
          strokeWidth={2}
          d="M3 5a2 2 0 012-2h3.28a1 1 0 01.948.684l1.498 4.493a1 1 0 01-.502 1.21l-2.257 1.13a11.042 11.042 0 005.516 5.516l1.13-2.257a1 1 0 011.21-.502l4.493 1.498a1 1 0 01.684.949V19a2 2 0 01-2 2h-1C9.716 21 3 14.284 3 6V5z"
        />
      </svg>
    ),
    label: "Telefon",
    value: "+994 12 123 45 67",
    sub: "7/24 Dəstək",
    href: "tel:+994121234567",
  },
  {
    icon: (
      <svg
        className="w-5 h-5"
        fill="none"
        stroke="currentColor"
        viewBox="0 0 24 24"
      >
        <path
          strokeLinecap="round"
          strokeLinejoin="round"
          strokeWidth={2}
          d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z"
        />
      </svg>
    ),
    label: "Email",
    value: "info@profitransport.az",
    sub: "info@profitransport.az",
    href: "mailto:info@profitransport.az",
  },
  {
    icon: (
      <svg
        className="w-5 h-5"
        fill="none"
        stroke="currentColor"
        viewBox="0 0 24 24"
      >
        <path
          strokeLinecap="round"
          strokeLinejoin="round"
          strokeWidth={2}
          d="M17.657 16.657L13.414 20.9a1.998 1.998 0 01-2.827 0l-4.244-4.243a8 8 0 1111.314 0z"
        />
        <path
          strokeLinecap="round"
          strokeLinejoin="round"
          strokeWidth={2}
          d="M15 11a3 3 0 11-6 0 3 3 0 016 0z"
        />
      </svg>
    ),
    label: "Ünvan",
    value: "Bakı, Azərbaycan",
    sub: "Nizami rayonu",
    href: "#",
  },
];

export default function StickySidebar() {
  return (
    <div className="lg:sticky lg:top-30   space-y-6">
      {/* Navigation */}
      <div className="bg-tertiary rounded-2xl p-6 lg:p-8">
        <h3 className="font-bold text-secondary mb-5 text-lg">Mündəricat</h3>
        <ul className="space-y-2.5">
          {navItems.map((item) => (
            <li key={item.href}>
              <a
                href={item.href}
                className="text-sm text-gray-500 hover:text-primary transition-colors flex items-center gap-2.5 py-1.5 group"
              >
                <span className="w-1.5 h-1.5 bg-gray-300 group-hover:bg-primary rounded-full transition-colors" />
                {item.label}
              </a>
            </li>
          ))}
        </ul>
      </div>

      {/* Contact Info Card */}
      <motion.div
        initial={{ opacity: 0, y: 20 }}
        animate={{ opacity: 1, y: 0 }}
        transition={{ duration: 0.5, delay: 0.1 }}
        className="bg-secondary rounded-2xl p-6 lg:p-8 text-white lg:block hidden"
      >
        <h4 className="font-bold text-lg mb-1">Bizimlə əlaqə</h4>
        <p className="text-sm text-gray-400 mb-6">
          Mütəxəssislərimiz sizə kömək etməyə hazırdır
        </p>

        {/* Contact Items */}
        <div className="space-y-4 mb-6">
          {contactInfo.map((item, i) => (
            <a
              key={i}
              href={item.href}
              className="flex items-start gap-3 group hover:bg-white/5 rounded-xl p-2 -mx-2 transition-colors"
            >
              <div className="w-10 h-10 bg-primary/20 rounded-xl flex items-center justify-center shrink-0 group-hover:bg-primary/30 transition-colors">
                <span className="text-primary">{item.icon}</span>
              </div>
              <div>
                <div className="text-xs text-gray-400 mb-0.5">{item.label}</div>
                <div className="text-sm font-medium text-white group-hover:text-primary transition-colors">
                  {item.value}
                </div>
                <div className="text-xs text-gray-500">{item.sub}</div>
              </div>
            </a>
          ))}
        </div>

        {/* WhatsApp Button */}
        <a
          href="https://wa.me/994121234567"
          target="_blank"
          rel="noopener noreferrer"
          className="flex items-center justify-center gap-2.5 w-full bg-[#25D366] hover:bg-[#128C7E] text-white py-3.5 rounded-xl font-semibold text-sm transition-all duration-300 hover:shadow-lg hover:shadow-[#25D366]/25 mb-6"
        >
          <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24">
            <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z" />
          </svg>
          WhatsApp ilə yaz
        </a>

        {/* Social Links */}
        <div className="pt-5 border-t border-white/10">
          <div className="text-xs text-gray-400 mb-3 uppercase tracking-wider font-medium">
            Sosial şəbəkələr
          </div>
          <div className="flex flex-wrap gap-2">
            {availableIcons.map((social) => (
              <a
                key={social.value}
                href={"#"}
                target="_blank"
                rel="noopener noreferrer"
                className={`size-10 shrink-0 rounded-xl bg-white/10 flex items-center justify-center text-gray-400 transition-all duration-300  hover:bg-primary hover:text-white`}
                title={social.value}
              >
                {renderSocialIcon({
                  iconName: social.value,
                })}
              </a>
            ))}
          </div>
        </div>
      </motion.div>
    </div>
  );
}
