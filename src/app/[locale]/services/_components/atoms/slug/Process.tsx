...
export default async function ProcessDetail({
  ...
  return (
    <div id="process" className="mt-16 scroll-mt-8">
      <div className="mb-8">
        {process.subTitle && (
        ...
        <h2
          title={process.title}
          className="text-3xl font-bold text-foreground mt-2 mb-4"
        >
          {process.title}
        </h2>
        ...
        <div className="space-y-8">
          {process.items.map((step: jsonItem, i: number) => (
            <div className="relative flex gap-6 pl-2" key={i}>
              ...
              <div className="surface-card p-6 flex-1 transition-colors duration-300">
                <strong className="font-bold text-lg mb-2 text-foreground block">
