...
import { WorkProcessItem } from "@/services/interface/type";
import { uptadeProcessImage } from "@/actions/client/process/process.controller";
...
export default function UpdateImageComponent({ existingData, refetch }: Props) {
  ...
  const { execute, isExecuting } = useAction(uptadeProcessImage, {
    onSuccess: async () => {
      await refetch();
