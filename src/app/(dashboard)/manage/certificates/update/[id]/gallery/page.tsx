...
import { CertificatesType, FileType } from "@/services/interface/type";
...
export default function UpdateGalleryPage() {
  ...
  const getCategoryWrapper = async () => {
    ...
    return {
      data: result as unknown as CertificatesType | undefined,
    };
  };

  const { data: existingData, refetch } = useServerQueryById<CertificatesType>(
    certificates_list,
    getCategoryWrapper,
    id,
    { locale: "en" },
  ...
  const { execute, isExecuting } = useAction(updateCertificatesGallery, {
